...
for test in $DIR/riscv-rvv-tests/isa/rv64uv-p-*; do
    [[ -d $test || ${test%%.dump} != $test || ${test%%.cout} != $test ]] && continue

#    if [[ -f "${test%.elf}${var:+.$var}.cout" ]]; then
#        echo "Skipping: ${test%.elf}${var:+.$var}.cout, Output files already exist."
#        continue
#    fi

    if [[ "$var" == "fulldyn" ]]; then
      extra="-Dv -L./c_emulator"
    fi
    if timeout 2000 $RISCVDIR/c_emulator/riscv_sim_RV64${var:+.$var} $extra -p $test > ${test%.elf}${var:+.$var}.cout 2>&1 && grep -q SUCCESS ${test%.elf}${var:+.$var}.cout; then
